...
FLT_MAX=100000000; % a large number 
FLT_MIN=1e-20;
load ./featfolder/featureval.mat; % load all the features
load walk_reward_weights.txt;  % load the learned weights
weights=walk_reward_weights;
img1 = imread('birdview_resize.jpg'); % load the image
figure(1);
imagesc(img1);
...
figure(2);
imagesc(Rshow);
%% Backward algorithm to compute Value function
if valmatmode==0
...
%% The computation of optimal Policy
na=9; % maximum number of actions
for i=1:na
   Pax{i}=zeros(nrow,ncol);  % policy matrix
end
...
%% Forward pass algorithm to compute forecasting distribution 
D=zeros(nrow,ncol);
N0=D;N1=D;
N0(startptx(1),startpty(1))=1;
% N0(startptx(2),startpty(2))=1;
niter=0;
while(1)
    niter=niter+1;
    N1=zeros(nrow,ncol);
